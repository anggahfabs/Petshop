...
<div class="flex justify-between items-center mb-6">
    <h1 class="text-3xl font-bold">Kotak Masuk</h1>
</div>
    ...
    <table class="w-full text-left border-collapse">
        <thead class="bg-gray-50 border-b">
            <tr>
                ...
                <th class="px-6 py-4 font-semibold text-gray-700">Pesan</th>
                    ...
                    <td class="px-6 py-4 flex items-center gap-3">
                        ...
                        @if(!$msg->is_read)
                            <form action="{{ route('admin.inbox.read', $msg) }}" method="POST">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="text-green-600 hover:text-green-800 font-medium">Tandai Dibaca</button>
                            </form>
                        @endif
                        <form action="{{ route('admin.inbox.destroy', $msg) }}" method="POST" onsubmit="return confirm('Hapus pesan ini?');">
                            @csrf
                            @method('DELETE')
...
<div id="viewModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-50">
    <div class="bg-white rounded-lg w-full max-w-lg p-6 shadow-xl">
        <h2 class="text-xl font-bold mb-4 text-gray-800">Detail Pesan</h2>
        ...
        <div class="flex justify-end mt-6">
            <button type="button" onclick="document.getElementById('viewModal').classList.add('hidden')" class="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700">Tutup</button>
        </div>
    </div>
</div>
