    ...
    <table class="w-full text-left border-collapse">
        <thead class="bg-gray-50 border-b">
            <tr>
                ...
                <th class="px-6 py-4 font-semibold text-gray-700">Products Count</th>
            ...
            @forelse($brands as $item)
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 flex items-center gap-3">
                        <button 
                            onclick="openEditModal({{ $item->id }}, '{{ addslashes($item->name) }}', {{ $item->is_active }})"
                            class="text-blue-600 hover:text-blue-800 font-medium"
                        >
                            Edit
                        </button>
                        <form action="{{ route('admin.brands.destroy', $item) }}" method="POST" onsubmit="return confirm('Delete this brand? Products associated will be unbranded.');">
                            @csrf
                            @method('DELETE')
...
<div id="createModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-50">
    <div class="bg-white rounded-lg w-full max-w-md p-6 shadow-xl transform transition-all">
        <h2 class="text-xl font-bold mb-4 text-gray-800">Add Brand</h2>
        <form action="{{ route('admin.brands.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
                <label class="block text-gray-700 mb-2 font-medium">Brand Name <span class="text-red-500">*</span></label>
                <input type="text" name="name" class="w-full border p-2 rounded focus:ring-2 focus:ring-blue-200 focus:outline-none" required>
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 mb-2 font-medium">Logo</label>
                <input type="file" name="image" class="w-full border p-2 rounded focus:ring-2 focus:ring-blue-200 focus:outline-none" accept="image/*">
            </div>
...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-50">
    <div class="bg-white rounded-lg w-full max-w-md p-6 shadow-xl">
        ...
        <form id="editForm" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 mb-2 font-medium">Brand Name <span class="text-red-500">*</span></label>
                <input type="text" name="name" id="edit_name" class="w-full border p-2 rounded focus:ring-2 focus:ring-blue-200 focus:outline-none" required>
            </div>
             <div class="mb-4">
                <label class="block text-gray-700 mb-2 font-medium">Change Logo</label>
